...
export default function CarGallery({ images, name }: { images: string[]; name: string }) {
  ...
  // Pantau perubahan slide dari Embla (termasuk saat di-swipe manual)
  useEffect(() => {
    if (!emblaApi) return;
    onSelect();
  ...
  useEffect(() => {
    ...
    // Hentikan interval jika komponen ditutup
      ...
      <div className="relative rounded-none border border-gray-200 overflow-hidden bg-gray-100 shadow-sm group">
        <div className="embla overflow-hidden" ref={emblaRef}>
          <div className="embla__container flex">
            {images.map((src, i) => (
              <div key={i} className="embla__slide relative flex-[0_0_100%] cursor-zoom-in" onClick={() => setLightbox(src)}>
                <img src={src} alt={`${name} ${i + 1}`} className="w-full h-[300px] md:h-[450px] object-cover" />
      ...
      <div className="flex gap-3 mt-4 overflow-x-auto pb-2 scrollbar-hide">
        {images.map((src, i) => (
          <button 
            key={i} 
            onClick={() => scrollTo(i)} 
            className={`shrink-0 w-20 h-20 overflow-hidden border-2 transition-all duration-300 rounded-none ${i === current ? "border-gray-900 opacity-100" : "border-transparent opacity-50 hover:opacity-100"}`}
          >
        ...
        {lightbox && (
          ...
          >
            <motion.img
              ...
              src={lightbox}
              alt={name}
              className="max-w-full max-h-[90vh] object-contain shadow-2xl rounded-none"
